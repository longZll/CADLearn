...
namespace DotNetARX
{
    ...
    public static class LayerTools
    {
        ...
        public static bool DeleteLayer(this Database db, string layerName)
        {
            //打开层表
            LayerTable lt=(LayerTable)db.LayerTableId.GetObject(OpenMode.ForRead);
            //图层名字校验---如果层名为0或Defpoints，则返回（这两个图层不能删除）
            ...
            //如果要删除的图层包含对象或者依赖外部参照，则返回（不能删除这些层）
            lt.GenerateUsageData();   //生成使用数据---对层表的用户数据进行更新,不更新的话可能出现 不依赖外部参照或包含对象的图层无法被删除的情况
            if (ltr.IsUsed) return false;  //如果要删除的图层包含对象，则返回（不能删除这个图层）
