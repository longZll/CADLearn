...
namespace HighlightRule
{
    ...
    {
        private static DrawRule drawRule;//定义规则重定义类  
        ...
        public void Initialize()
        {
            ...
            if (hightLightRule == null)
            {
                ...
                //为直线添加亮显重定义
                Overrule.AddOverrule(RXObject.GetClass(typeof(Line)), hightLightRule, false);
                //添加扩展数据过滤，只重定义有Door扩展数据的直线
                hightLightRule.SetXDataFilter(appName);
            }
        ...
        public void Register(Type typename, Overrule rule, string appName)
        {
            if (rule != null)
            {
                RXObject.GetClass(typename).Create();
