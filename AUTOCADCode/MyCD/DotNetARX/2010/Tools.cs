...
namespace DotNetARX
{
    ...
    public static partial class Tools
    {
        ...
        /// </summary>
        /// <param name="ids">要亮显的实体的Id集合</param>
        public static void HighlightEntities2(this ObjectIdCollection ids)
        {
            if (ids.Count == 0) return;
            //var trans = ids[0].Database.TransactionManager;
        ...
        public static void UnHighlightEntities(this ObjectIdCollection ids)
        {
            ...
            //在没有选择实体的情况下，如果尝试对空的实体ID集合进行操作，会导致程序抛出异常。在原始代码中，没有提供对此情况的处理，因此需要添加对空集合的检查。
            if (ids == null || ids.Count == 0) return;

            // 获取第一个实体的数据库，并获取其事务管理器
            var trans =ids[0].Database.TransactionManager;
