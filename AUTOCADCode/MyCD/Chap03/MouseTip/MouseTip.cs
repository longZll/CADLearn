...
namespace MouseTip
{
    public class MouseTip
    {
        ...
        static string blockName="";         //用来存储鼠标停留处的块名
                ...
                if (paths.Length > 0)
                {
                    ...
                    if (blockRef != null)   //如果鼠标停留处为块参照
                    {
                        ...
                        if (blockName != btr.Name)
                        {
                            ...
                            {
                                BlockReference blockRef_2 = trans.GetObject(id, OpenMode.ForRead) as BlockReference;
                                if (blockRef_2 != null)
                                {
                                    blockIds.Add(id);       // 添加块参照的ObjectId到集合中
                                    blockRef_2.Highlight(); // 高亮显示块参照
                                }
                            }
                        }
